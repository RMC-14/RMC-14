...
public sealed class UniqueActionSystem : EntitySystem
{
    ...
    {
        if(!_entityManager.TryGetComponent<HandsComponent>(userUid, out HandsComponent? handsComponent) ||
            !_entityManager.TryGetComponent<UniqueActionComponent>(handsComponent.ActiveHandEntity, out UniqueActionComponent? uniqueActionComponent))
            return;
