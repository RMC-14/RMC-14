...
public abstract class SharedEvacuationSystem : EntitySystem
{
    ...
    private void OnEvacuationComputerExamined(Entity<EvacuationComputerComponent> ent, ref ExaminedEvent args)
    {
        if (ent.Comp.MaxMobs is { } maxMobs)
        {
            ...
            {
                args.PushMarkup(Loc.GetString("rmc-evac-pod-max-count", ("maxmobs", maxMobs)));
            }
        }
    }
    ...
    private void OnEvacuationComputerUIOpenAttempt(Entity<EvacuationComputerComponent> ent, ref ActivatableUIOpenAttemptEvent args)
    {
        ...
        {
            EvacuationComputerMode.Disabled => Loc.GetString("rmc-evac-has-not-started"),
            EvacuationComputerMode.Ready => "",
            EvacuationComputerMode.Travelling => Loc.GetString("rmc-evac-pod-has-already-launched"),
            EvacuationComputerMode.Crashed => Loc.GetString("rmc-evac-pod-has-crashed"),
    ...
    private void OnEvacuationPumpExamined(Entity<EvacuationPumpComponent> ent, ref ExaminedEvent args)
    {
        ...
        using (args.PushGroup(nameof(EvacuationPumpComponent)))
        {
            ...
            if (progress < 25)
                args.PushMarkup(Loc.GetString("rmc-evac-progress-less-25"));
            else if (progress < 50)
                args.PushMarkup(Loc.GetString("rmc-evac-progress-less-50"));
            else if (progress < 75)
                args.PushMarkup(Loc.GetString("rmc-evac-progress-less-75"));
            else if (progress < 100)
                args.PushMarkup(Loc.GetString("rmc-evac-progress-less-100"));
            else
                args.PushMarkup(Loc.GetString("rmc-evac-progress-less-100"));
        }
    }
    ...
    private void OnLifeboatComputerUIOpenAttempt(Entity<LifeboatComputerComponent> ent, ref ActivatableUIOpenAttemptEvent args)
    {
        ...
        args.Cancel();
        _popup.PopupClient(Loc.GetString("rmc-evac-has-not-started"), ent, args.User, PopupType.SmallCaution);
    }
    ...
    private void OnEvacuationComputerLaunch(Entity<EvacuationComputerComponent> ent, ref EvacuationComputerLaunchBuiMsg args)
    {
        ...
        if (ent.Comp.MaxMobs is { } maxMobs)
        {
            ...
            {
                _popup.PopupPredicted(Loc.GetString("rmc-evac-pod-so-much"), ent, null, PopupType.LargeCaution);
    ...
    public void ToggleEvacuation(SoundSpecifier? startSound, SoundSpecifier? cancelSound, EntityUid? map)
    {
        ...
        if (progress.Enabled)
        {
            _marineAnnounce.AnnounceARESStaging(
                null,
                Loc.GetString("rmc-evac-attention"),
                startSound
            );
        ...
        {
            _marineAnnounce.AnnounceARESStaging(null, Loc.GetString("rmc-evac-cancelled"), cancelSound);
    ...
    private void ProcessEvacuation()
    {
        ...
        while (query.MoveNext(out var uid, out var progress))
        {
            ...
            if (!progress.StartAnnounced)
            {
                ...
                areas.Append(
                    Loc.GetString("rmc-evac-low-orbit"));
            ...
            if (progress.Progress >= progress.NextAnnounce)
            {
                ...
                {
                    var marineAnnounce = Loc.GetString("rmc-evac-fuel-percent-marine", ("percentage", percentage));
                    if (offAreas.Length == 0)
                        marineAnnounce += Loc.GetString("rmc-evac-fuel-percent-marine-all-areas");
                    else
                        marineAnnounce += Loc.GetString("rmc-evac-fuel-percent-marine-not-all-areas", ("areas", offAreas));
                ...
                {
                    _marineAnnounce.AnnounceARESStaging(null, Loc.GetString("rmc-evac-fuel-percent-marine-100"));
                    _xenoAnnounce.AnnounceAll(default, Loc.GetString("rmc-evac-fuel-percent-xeno-100"));
                ...
                else if (progress.Progress >= progress.Required * 0.75)
                {
                    ...
                    var xenoAnnounce = Loc.GetString("rmc-evac-fuel-percent-xeno-75");
                    if (onAreas.Length > 0)
                        xenoAnnounce += Loc.GetString("rmc-evac-fuel-percent-xeno-areas", ("areas", onAreas));
                ...
                else if (progress.Progress >= progress.Required * 0.5)
                {
                    ...
                    var xenoAnnounce = Loc.GetString("rmc-evac-fuel-percent-xeno-50");
                    if (onAreas.Length > 0)
                        xenoAnnounce += Loc.GetString("rmc-evac-fuel-percent-xeno-areas", ("areas", onAreas));
                ...
                {
                    var marineAnnounce = Loc.GetString("rmc-evac-fuel-percent-marine-25");
                    if (offAreas.Length == 0)
                        marineAnnounce += Loc.GetString("rmc-evac-fuel-percent-marine-all-areas");
                    else
                        marineAnnounce += Loc.GetString("rmc-evac-fuel-percent-marine-not-all-areas", ("areas", offAreas));
                    ...
                    var xenoAnnounce = Loc.GetString("rmc-evac-fuel-percent-xeno-25");
                    if (onAreas.Length > 0)
                        xenoAnnounce += Loc.GetString("rmc-evac-fuel-percent-xeno-areas", ("areas", onAreas));
