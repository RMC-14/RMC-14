...
namespace Content.Client._RMC14.UserInterface.Systems.Ghost.Controls
{
    ...
    public sealed partial class RMCGhostTargetWindow : DefaultWindow
    {
        ...
        private sealed class WarpGroup
        {
            ...
            public List<(string displayName, NetEntity Entity, bool IsWarpPoint, string? DisplayJob, int FollowerCount, string? HealthState, int HealthPercent)> Warps = new();
        ...
        public void UpdateWarps(IEnumerable<RMCGhostWarp> warps)
        {
            ...
            // --- Custom groups (factions) ---
            var allGroupProtos = new Dictionary<ProtoId<NpcFactionPrototype>, NpcFactionPrototype>();
            ...
            var allWarpGroups = new Dictionary<ProtoId<NpcFactionPrototype>, WarpGroup>();
            ...
            {
                BuildGroupTree(proto.ID, allGroupProtos, allWarpGroups, childToParent, new Stack<ProtoId<NpcFactionPrototype>>());
            }
            ...
            {
                var entry = (warp.DisplayName, warp.Entity, warp.IsWarpPoint, warp.DisplayJob, warp.FollowerCount, warp.HealthState, warp.HealthPercent);
                ...
                {
                    ghosts.Warps.Add((warp.DisplayName, warp.Entity, warp.IsWarpPoint, null, warp.FollowerCount, warp.HealthState, warp.HealthPercent));
                    continue;
                }
                // 4. Custom group (only for alive, non-ghost, non-WarpPoint)
                if (_entityManager.TryGetComponent<NpcFactionMemberComponent>(entity, out var factionComp))
                {
                    foreach (var faction in factionComp.Factions)
                    {
                        if (!allWarpGroups.TryGetValue(faction, out var warpGroup))
                            continue;
        ...
        private WarpGroup? BuildGroupTree(
            ...
            Dictionary<ProtoId<NpcFactionPrototype>, NpcFactionPrototype> allGroupProtos,
            Dictionary<ProtoId<NpcFactionPrototype>, WarpGroup> allWarpGroups,
        ...
        private void AddContent()
        {
            ...
            foreach (var group in _warpGroups)
            {
                ...
                if (!HasContent(group))
                    continue;
                AddGroupContent(group, ContentContainer, spriteSystem, false);
            }
        }
        ...
        /// </summary>
        private void AddGroupContent(WarpGroup group, Container parent, SpriteSystem spriteSystem, bool isSubgroup)
        {
            ...
            if (group.Warps.Count > 0)
            {
                ...
                foreach (var (name, warpTarget, isWarpPoint, job, followerCount, healthState, healthPercent) in group.Warps)
                {
                    var button = CreateWarpButton(name, warpTarget, isWarpPoint, job, followerCount, healthState, healthPercent, spriteSystem, group, ButtonIsVisible);
            ...
            if (group.Subgroups != null && group.Subgroups.Count > 0)
            {
                ...
                foreach (var subgroup in group.Subgroups)
                {
                    ...
                    if (HasContent(subgroup))
                    {
                        AddGroupContent(subgroup, subgroupsBox, spriteSystem, true);
                    }
                }
        ...
        /// </summary>
        private Button CreateWarpButton(string name, NetEntity warpTarget, bool isWarpPoint, string? job, int followerCount, string? healthState, int healthPercent,
            ...
            WarpGroup group, Func<Button, bool> isVisibleCheck)
        {
            ...
            int? tooltipHealthPercent = null;
            if (!isWarpPoint)
            {
                AddIconsToButtonContainer(buttonContainer, warpTarget, healthState, spriteSystem);
                if (healthPercent >= 0)
                    tooltipHealthPercent = healthPercent;
            }
            ...
            if (!isWarpPoint)
                AddGhostCounter(buttonContainer, followerCount);
            ...
            if (!isWarpPoint)
                button.ToolTip = GetTooltip(warpTarget, name, job, tooltipHealthPercent);
        ...
        /// </summary>
        private void AddIconsToButtonContainer(BoxContainer buttonContainer, NetEntity warpTarget, string? healthState, SpriteSystem spriteSystem)
        {
        ...
        /// </summary>
        private string GetTooltip(NetEntity warpTarget, string name, string? job, int? healthPercent)
        {
            var entity = _entityManager.GetEntity(warpTarget);
            ...
            {
                tooltipText += $"\n{Loc.GetString("rmc-ghost-target-window-tooltip-health", ("health", healthPercent.Value))}";
            }
