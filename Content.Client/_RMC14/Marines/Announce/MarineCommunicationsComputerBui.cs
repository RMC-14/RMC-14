...
public sealed class MarineCommunicationsComputerBui(EntityUid owner, Enum uiKey) : BoundUserInterface(owner, uiKey)
{
    ...
    public void OnStateUpdate()
    {
        ...
        if (EntMan.TryGetComponent(Owner, out MarineControlComputerComponent? evaccomputer))
        {
            ...
            if (_confirmingEvacuation)
                _window.EvacuationButton.Text = Loc.GetString("rmc-command-comp-evac-send");
            else
                _window.EvacuationButton.Text = evaccomputer.Evacuating ? Loc.GetString("rmc-command-comp-cancel-evac"): Loc.GetString("rmc-command-comp-initiate-evac");
