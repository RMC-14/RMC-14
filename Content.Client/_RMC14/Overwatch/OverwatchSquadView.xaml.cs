...
public sealed partial class OverwatchSquadView : Control
{
    ...
    public void UpdateResults(
        ...
        bool showDead,
        bool showHidden,
        IReadOnlyList<OverwatchListEntry> entries,
        OverwatchConsoleComponent console)
    {
        ...
        for (var i = 0; i < Names.ChildCount; i++)
        {
            ...
            if (location != null)
            {
                if (i >= entries.Count || entries[i].Location != location)
                {
                    MakeViewVisible(i, false);
                    continue;
                }
            ...
            if (!showDead)
            {
                if (i >= entries.Count || !entries[i].IsAlive)
                {
                    MakeViewVisible(i, false);
                    continue;
                }
            ...
            if (!showHidden)
            {
                ...
                if (i >= entries.Count || hidden.Contains(entries[i].Id))
                {
                    MakeViewVisible(i, false);
                    continue;
                }
